			...
			p.mouseDragged = () =>
			{
				...
				switch(mode) {
					...
					case "stars":
						...
						let allStars = ['.','⋆','·','⊹','*','˚','✦','✵','+','✺','✹','✫','✧']
						let starIndex = Math.floor(Math.random()*allStars.length)
					...
					case "smudge":
						...
						for (var theta = 0; theta < 360; theta += 30)
						{
							...
							for (var r = 0; r < maxR; r++)
							{
								...
								// new point
								let [x2, y2] = vector.getAddPolar([x, y], r, theta * (360 / Math.PI));
					...
					case "squiggles":
						...
						let lastMouse = [mousePositions[mousePositions.length-2][0], mousePositions[mousePositions.length-2][1]];
						...
						let vecMouse = [lastMouse[0]-currMouse[0], lastMouse[1]-currMouse[1]];
						...
						for (var i = 0; i <= curves; i++)
						{
							...
							v = [v[0] * Math.cos(theta) - v[1] * Math.sin(theta),
							...
							p.bezier(p0[0], p0[1], cp0[0], cp0[1], cp1[0], cp1[1], p1[0], p1[1]);
						}
