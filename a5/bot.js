class CoffeeBot
{
	...
	respondTo(s)
	{
		if (!(s.includes("wait") || s.includes("care") || s.includes("play"))) return "..."
		...
		if (s.includes("wait"))
		{
			response += `${this.plant}` + this.grammar.flatten(" #waitVerb# #waitAdverb#. ")
		...
		else
		{
			...
			{
				if (this.healthAmount == this.maxHealth) response += "max health. "

				else
				{
					this.healthAmount += 2
					if (!s.includes("play") && this.happyAmount > 0) this.happyAmount -= 1
					response += this.grammar.flatten("you #careVerb# ") + `${this.plant}` + this.grammar.flatten(" #careAdverb#. ")
				}
			...
			{
				if (this.happyAmount == this.maxHappy) response += "max playfulness. "

				else
				{
					if (this.healthAmount <= 2)
					{
						response += `${this.plant}` + " doesn't feel like playing. "
					}
					else if (this.healthAmount <= 6)
					{
						this.happyAmount += 2
						response += this.grammar.flatten("you #playVerb# ") + `${this.plant}` + this.grammar.flatten(" #playAdverb#. ")
					}
					else
					{
						this.happyAmount += 3
					...
					if (!s.includes("care") && this.healthAmount > 0) this.healthAmount -= 1
				} 
			}
		}
		...
		if ((oldHealth < 9 || oldHappy < 9) && (this.healthAmount >= 9 && this.happyAmount >= 9))
		{
			this.plant = `the ${this.grammar.flatten("#plant#")}`
			response += `${oldPlant} bloomed into a ${this.plant.substring(3)}!`
		}
		else if ((oldHealth >= 9 && oldHappy >= 9) && (this.healthAmount < 9 || this.happyAmount < 9))
		{
			this.plant = "the plant"
			response += `${oldPlant} transformed back to a ${this.plant.substring(3)}.`
		}
