...
const faceIndexes =
{
	...
	mask: [
		197, 196, 100, 123, 147, 213, 192, 210, 211,  32, 208,
		199, 428, 262, 431, 430, 416, 433, 376, 352, 329, 419, 197
	],
	...
	{
		nose: [
			[ 67,  69,  66,  65, 221, 189, 245, 188, 174, 236, 134, 220, 239, 20, 242, 141, 94],
		...
		eyeShadow: [
			231, 230, 229, 228, 31, 226, 156, 124, 113, 225, 224,
			35, 46, 223, 222, 71, 70, 53,
	...
	{
		nose:   [
			[297, 299, 296, 295, 441, 413, 412, 399, 456, 363, 440, 459, 250, 462, 370, 94],
		...
		eyeShadow: [
			451, 450, 449, 448, 261, 446, 383, 353, 342, 445, 444,
			265, 276, 443, 442, 301, 300, 283,
			463, 414, 286, 258, 257, 259, 260, 467, 359, 255, 339, 254, 253, 252, 256
		]
	}]
}
...
for (var i = 0; i < 2; i++) {
	...
	face.sides[i] =
	{
		...
		eyeShadow: faceIndexes.sides[i].eyeShadow.map(index => {
			return face.points[index]
		}),
...
face.mask = faceIndexes.mask.map(index => face.points[index])
face.maskStrings = faceIndexes.maskStrings.map(mstr => mstr.map(index => face.points[index]))
